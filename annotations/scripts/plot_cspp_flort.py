...
ims = [oi.CE01ISSP_FLORT,oi.CE02SHSP_FLORT,oi.CE06ISSP_FLORT,oi.CE07SHSP_FLORT]
...
def plot_data(df,variables,fdir,figname):
    ...
    df2 = oh.df_drop_range(df2,'fluorometric_chlorophyll_a',0,50)
    df2 = oh.df_drop_range(df2,'fluorometric_cdom',0,375)   
    df2 = oh.df_drop_range(df2,'optical_backscatter',0,3)  
    ...
    for i in range(len(vs)):           
        if vs[i] == 'fluorometric_chlorophyll_a':
            vmin = 0
            vmax = 10
        elif vs[i] == 'fluorometric_cdom':    
            vmin = 0
            vmax = 5
        elif vs[i] == 'optical_backscatter':
            vmin = 0
            vmax = 0.15
        ...
        cax = fig.add_axes([0.91,ypos,0.01,0.15])
